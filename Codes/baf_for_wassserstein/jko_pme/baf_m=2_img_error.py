...
plt.loglog(t, img_tau04, label=r"$\tau = 0.4$")

t = np.arange(0, 2.1, 0.1)
plt.loglog(t, img_tau01, label=r"$\tau = 0.1$")
plt.loglog(t, img_tau0025, label=r"$\tau = 0.025$")
plt.loglog(t, img_tau000625, label=r"$\tau = 0.00625$")
plt.xlabel("t")
plt.ylabel("error = computed - exact")
plt.legend()
plt.savefig(f"{image_root}error_baf_log.png")
plt.close()