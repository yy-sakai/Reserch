...
#phi = tau * phi
phi = -(m / (m - 1)) * nu ** (m - 1) * tau * 0.001  #(tau = 0.001) \phi_0 = \phi^(0) = \delta U(\nu^(0)) =  \delta U(\rho^(0))
...
for k in range(200):
    ...
    sigma1, J, H1_sq, phi, psi, pfwd  = ascent1(phi, psi, mu, nu, sigma1)  # phi = phi_{k + 1/2}, psi = psi_{k + 1/2}
    ...
    sigma2, J, H1_sq, psi, phi, pfwd = ascent2(psi, phi, nu, mu, sigma2)
